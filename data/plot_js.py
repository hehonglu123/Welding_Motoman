...
def main():
	...
	for i in range(num_layers):
		if i % 2 == 0:
			...
			curve_sliced_js.append(np.flip(np.loadtxt(data_dir+'curve_sliced_js/MA2010_js'+str(i)+'.csv',delimiter=','),axis=0))
			positioner_js.append(np.flip(np.loadtxt(data_dir+'curve_sliced_js/D500B_js'+str(i)+'.csv',delimiter=','),axis=0))
