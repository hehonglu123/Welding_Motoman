...
with open('recorded_data/slice_30_0_flir.pickle', 'rb') as file:
    ir_recording = pickle.load(file)

ir_ts=np.loadtxt('recorded_data/slice_30_0_flir_ts.csv', delimiter=',')
...
for i in range(len(ir_recording)):
    ...
    ir_normalized=np.clip(ir_normalized * 3, 0, 255)
    ...
    cv2.imshow("IR Recording", np.hstack((ir_bgr, color_bar_image)))
