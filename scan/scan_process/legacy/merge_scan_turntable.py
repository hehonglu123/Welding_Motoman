...
scan_per_pose=1 # take 3 scan every pose
...
T_R1_R2=np.loadtxt(config_dir+'MA1440_pose - Copy.csv',delimiter=",", dtype=np.float64)
T_R1_table=np.loadtxt(config_dir+'D500B_pose.csv',delimiter=",", dtype=np.float64)
...
# for i in range(5):
	...
	table_rot_k = np.matmul(curve_R[i].T,[0,0,1])
	points = np.transpose(np.matmul(curve_R[i],np.matmul(rot(-table_rot_k,joints_p[i,-1]-joints_p[0,-1]).T,np.transpose(points))))+curve[i]
