...
def main():
    ...
    for i in range(len(slice_all)):
        for x in range(len(slice_all[i])):
            ...
            if i not in range(20,35) and i not in range(36,50):
                if i==35:
                    ax.plot3D(slice_all[i][x][::5*vis_step,0],slice_all[i][x][::5*vis_step,1],slice_all[i][x][::5*vis_step,2],'r.-')

                    surf_norms=[]
                    for j in range(len(slice_all[i][x])):
                        indices=np.argsort(np.linalg.norm(stl_pc-slice_all[i][x][j],axis=1))[:10]
                        surf_norm=fit_plane(stl_pc[indices])[0]
                        if surf_norm@np.array([0,1,0])<0:
                            surf_norm=-surf_norm
                        surf_norms.append(surf_norm)

                    surf_norms=np.array(surf_norms)
                    #surface normal
                    ax.quiver(slice_all[i][x][::5*vis_step,0],slice_all[i][x][::5*vis_step,1],slice_all[i][x][::5*vis_step,2],surf_norms[::5*vis_step,0],surf_norms[::5*vis_step,1],surf_norms[::5*vis_step,2],length=10,color='r')
                    #curve normal
                    ax.quiver(slice_all[i][x][::5*vis_step,0],slice_all[i][x][::5*vis_step,1],slice_all[i][x][::5*vis_step,2],curve_normal_all[i][x][::5*vis_step,0],curve_normal_all[i][x][::5*vis_step,1],curve_normal_all[i][x][::5*vis_step,2],length=10,color='g')
                    #curve tangent
                    diff=np.gradient(slice_all[i][x][::5*vis_step],axis=0)
                    diff=diff/np.linalg.norm(diff,axis=1).reshape(-1,1)
                    ax.quiver(slice_all[i][x][::5*vis_step,0],slice_all[i][x][::5*vis_step,1],slice_all[i][x][::5*vis_step,2],diff[:,0],diff[:,1],diff[:,2],length=10,color='b')
                    
                else:
                    ax.plot3D(slice_all[i][x][::vis_step,0],slice_all[i][x][::vis_step,1],slice_all[i][x][::vis_step,2],'k.-')
