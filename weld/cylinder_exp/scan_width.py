...
from dual_robot import form_relative_path
...
def main():
	...
	# q_init_table=client.getJointAnglesDB(positioner.pulse2deg) ## init table
	...
	#plot in 3d for visualization
	# _,_,_,_,relative_p,relative_R = form_relative_path(np.array(q_bp1).squeeze(),np.array(q_bp2).squeeze(),robot2,positioner)
	pose2_all=robot2.fwd(np.array(q_bp1).squeeze())
	relative_p=pose2_all.p_all
	relative_R=pose2_all.R_all
	
	fig = plt.figure()
	ax = fig.add_subplot(111, projection='3d')
	ax.plot(relative_p[:,0],relative_p[:,1],relative_p[:,2])
	ax.quiver(relative_p[:,0],relative_p[:,1],relative_p[:,2],relative_R[:,0,-1],relative_R[:,1,-1],relative_R[:,2,-1],color='r')
	plt.show()
