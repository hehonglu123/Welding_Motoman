...
def main():
	...
	layer_height=1.1
	v_layer=10
	#edge params, 1cm per seg
	feedrates_edge=[70,80,90,100,100,100,100,90,80,70]
	vs_edge=[7,8,9,10,10,10,10,9,8,7]
	jobs_edge=(np.array(feedrates_edge)/10+job_offset).astype(int)	
	q_all=[]
	v_all=[]
	cond_all=[]
	primitives=[]
	...
	for i in range(2,62):
		...
		p_waypoints=np.linspace(p1,p2,len(feedrates_edge)+1)
		q_waypoints=robot.find_curve_js(p_waypoints,[R]*len(p_waypoints),q_seed)
		
		if i==2:	#if start of first normal layer
			q_all.extend(q_waypoints)
			v_all.extend([1]+vs_edge)
			primitives.extend(['movej']+['movel']*len(vs_edge))
			cond_all.extend([0]+jobs_edge.tolist())
		else:
			q_all.extend(q_waypoints[1:])
			v_all.extend(vs_edge)
			primitives.extend(['movel']*len(vs_edge))
			cond_all.extend(jobs_edge.tolist())
	...
	recorded_dir='../../../recorded_data/wallbf_%iipm_v%i_multiseg/'%(feedrate,v_layer)
	os.makedirs(recorded_dir,exist_ok=True)
	np.savetxt(recorded_dir+'weld_js_exe.csv',np.array(js_recording),delimiter=',')
	rr_sensors.save_all_sensors(recorded_dir)
	np.savetxt(recorded_dir+'mutliseg_params.csv',np.array([feedrates_edge,vs_edge]).T,delimiter=',')
