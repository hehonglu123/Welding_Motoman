...
def read_and_calculate_mse(model_dir):
    # Read the CSV files
    ...
    batch_size = test_dh_error.shape[0] // 40
    
    # Reshape errors to (batch, 40, 1) each
    reshaped_dh = test_dh_error.reshape(batch_size, 40, 1)
    reshaped_dw = test_dw_error.reshape(batch_size, 40, 1)
...
if __name__ == "__main__":
    model_dir = "model_20250717_140631/"  # Current directory, change as needed
    # model_dir = "model_20250715_122716/"  # Current directory, change as needed
    testing_loss, combined_error, mse = read_and_calculate_mse(model_dir)

    print("Min testing loss:", np.min(testing_loss))
