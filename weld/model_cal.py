...
for dataset in datasets:
    ...
    if dataset=='baseline':
        ...
        data_dir = '../data/wall_weld_test/ER4043_model_100_ipm_incremental_Velocities_07_17_2023/'
...
# get fit  rmse
y_fit = p(log_x)
rmse = np.sqrt(np.mean((np.exp(log_y)-np.exp(y_fit))**2))
rmse_log = np.sqrt(np.mean((log_y-y_fit)**2))
print('loglog-rmse',rmse)
print('loglog-rmse_log',rmse_log)

# 使用 linspace 函数创建用于绘图的 x 值数组
x_fit = np.linspace(log_x.min(), log_x.max(), 500)
# 使用多项式对象计算出 y 值
y_fit = p(x_fit)
plt.plot(log_x, log_y, 'o',label = r'$\log(\Delta h)$ Vs. $\log$(Velocity) Scatter')
plt.plot(x_fit, y_fit, '-',label = '100ipm_Model Fit Curve')  # 使用loglog函数绘制双对数图
plt.xlabel(r'$\log(v)$ (log(mm/s))',fontsize=18)
plt.ylabel(r'$\log(\Delta h)$ (log(mm))',fontsize=18)
...
plt.grid(True)
# Customize legend
legend = plt.legend(fontsize = 12, loc="upper right")  # Adjust fontsize as needed and specify location
