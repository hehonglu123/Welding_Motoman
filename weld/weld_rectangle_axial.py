...
for i in range(2,3): #layers
    if i%2==0:
        ...
        p_end=np.array([x_end,y_int,z_int])
    for n in range(6,7): # segments max: 7
        if i%2==0:
            p_start=np.array([x_int+(12*(n)),y_int,z_int])
            p_end=np.array([x_int+(12*(n)),y_end,z_int])
            p1=p_start+np.array([0,0,i*layer_height])
            p2=p_end+np.array([0,0,i*layer_height])
            p3=p_end+np.array([6,0,i*layer_height])
            p4=p_start+np.array([6,0,i*layer_height])
            p5=p_start+np.array([12,0,i*layer_height])
            print('p1,p2,p3,p4,p5:',p1,p2,p3,p4,p5)
            p_start=np.array([x_int+(12*(n+1)),y_int,z_int])
            p_end=np.array([x_int+(12*(n+1)),y_end,z_int])
        else:
            p_start=np.array([x_end+(-12*(n)),y_end,z_int])
            p_end=np.array([x_end+(-12*(n)),y_int,z_int])
            p1=p_start+np.array([0,0,i*layer_height])
            p2=p_end+np.array([0,0,i*layer_height])
            p3=p_end+np.array([-6,0,i*layer_height])
            p4=p_start+np.array([-6,0,i*layer_height])
            p5=p_start+np.array([-12,0,i*layer_height])
            print('p1,p2,p3,p4,p5:',p1,p2,p3,p4,p5)
            p_start=np.array([x_end+(-12*(n+1)),y_end,z_int])
            p_end=np.array([x_end+(-12*(n+1)),y_int,z_int])
        q_1=robot.inv(p1,R,q_seed)[0]
        q_2=robot.inv(p2,R,q_seed)[0]
        q_3=robot.inv(p3,R,q_seed)[0]
        q_4=robot.inv(p4,R,q_seed)[0]
