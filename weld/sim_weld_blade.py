...
for i in range(num_baselayers):
	curve_sliced_base_js.append(np.loadtxt(data_dir+'curve_sliced_js/MA2010_base_js'+str(i)+'.csv',delimiter=','))
	positioner_base_js.append(np.loadtxt(data_dir+'curve_sliced_js/D500B_base_js'+str(i)+'.csv',delimiter=','))
...
# 	time.sleep(10)
t.viewer_trajectory_dual(robot.robot_name,positioner.robot_name,curve_sliced_js[0][::20],positioner_js[0][::20])
