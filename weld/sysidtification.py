...
for dataset in datasets:
    ...
    height_only_y = []
    height_only_y = [np.array([item[1] for item in sublist]) for sublist in all_profile_height]


    # print('height_only_y',height_only_y)
    lengths = [len(arr) for arr in height_only_y]
    print(lengths)
        # 删除前两个元素
    height_only_y = height_only_y[2:]

    # 获取最小长度
    min_length = min(len(arr) for arr in height_only_y)

    # 调整每个数组的长度
    for i in range(len(height_only_y)):
        while len(height_only_y[i]) > min_length:
            # 如果数组长度大于最小长度，根据长度超出的量决定删除元素的位置
            if len(height_only_y[i]) - min_length == 1:
                # 长度大于1，只删除第一个元素
                height_only_y[i] = height_only_y[i][1:]
            else:
                # 长度大于2，删除第一个和最后一个元素
                height_only_y[i] = height_only_y[i][1:-1]
    lengths = [len(arr) for arr in height_only_y]
    print(lengths)
    for i in range(len(height_only_y)):
        height_only_y[i] = height_only_y[i][70:-70]
        # 创建存储差值的列表
    diff_arrays = []
    for arr in height_only_y:
        average = np.mean(arr)
        diff = average + 1.16 - arr
        diff_arrays.append(diff)

    # 创建存储每两个数组之间的差值的列表
    inter_arrays_diff = []
    for i in range(len(height_only_y)-1):
        diff = height_only_y[i+1] - height_only_y[i]
        inter_arrays_diff.append(diff)
    # print('diff_arrays',diff_arrays)
    print('len(diff_arrays)',len(diff_arrays))
    # print('inter_arrays_diff',inter_arrays_diff)
    print('len(inter_arrays_diff)',len(inter_arrays_diff))
    # # 打印两个列表
    # for diff in diff_arrays:
    #     print(diff)
    #     print(len(diff))
        
    # for diff in inter_arrays_diff:
    #     print(diff)
    #     print(len(diff))
    # plt.figure(figsize=(6, 6))

    # # 绘制diff_arrays的前40组数据
    # for i in range(40):
    #     plt.scatter(range(len(diff_arrays[i])), diff_arrays[i], color='blue')

    # # 绘制inter_arrays_diff
    # for i in range(len(inter_arrays_diff)):
    #     plt.scatter(range(len(inter_arrays_diff[i])), inter_arrays_diff[i], color='red')

    # plt.show()
    # for i in range(len(diff_arrays)):
    #     diff_arrays[i] = diff_arrays[i][20:]
    # for i in range(len(inter_arrays_diff)):
    #     inter_arrays_diff[i] = inter_arrays_diff[i][:-20]



    # 准备输入信号和输出信号的数据
    input_signals = inter_arrays_diff  # 输入信号的数据，包括15组数据，每组数据是一个数组
    output_signals =diff_arrays   # 输出信号的数据，对应于输入信号的15组数据

    # 将数据分为训练集和测试集
    train_input = np.array(input_signals[:30])
    train_output = np.array(output_signals[:30])
    test_input = np.array(input_signals[30:])
    test_output = np.array(output_signals[30:])


    # 定义系统模型函数
    def system_model(x, *params):
        # params 包含待估计的模型参数
        # 在这里定义适当的系统模型方程，并根据输入 x 和 params 返回预测的输出
        return predicted_output

    # 使用训练数据进行系统辨识
    params_initial = [...]  # 模型参数的初始猜测值
    params_opt, _ = curve_i(system_model, train_input, train_output, p0=params_initial)

    # 使用估计得到的参数预测和验证测试数据
    predicted_output = system_model(test_input, *params_opt)

    # 计算预测结果与实际结果之间的误差
    error = predicted_output - test_output
    ...
    # for i in range(40):
    #     plt.figure(figsize=(6, 6))
    #     plt.scatter(inter_arrays_diff[i], diff_arrays[i], color='blue', label='tip_d')
    #     # 添加图例
    #     plt.legend()
    ...
    #     plt.show()
    # exit()
    # 遍历diff_arrays和inter_arrays_diff
    # for i in range(40):
    #     plt.figure(figsize=(6, 6))
    #     # 绘制diff_arrays的一组数据，并添加标签'tip_d'
    #     plt.scatter(range(len(diff_arrays[i])), diff_arrays[i], color='blue', label='tip_d')
    #     # 如果inter_arrays_diff的索引在有效范围内，就绘制它的一组数据，并添加标签'delta_h'
    #     if i < len(inter_arrays_diff):
    #         plt.scatter(range(len(inter_arrays_diff[i])), inter_arrays_diff[i], color='red', label='delta_h')
    #     # 添加图例
    #     plt.legend()
    #     plt.xlabel('lamda (0.1mm)')
