...
center=np.array([1648,-1240,-229])
x_offset=np.array([35,0,0])
layer_height=1
R=np.array([[ 0.7071, -0.7071, -0.    ],
			[-0.7071, -0.7071,  0.    ],
			[-0.,      0.,     -1.    ]])
third_layer=[center-base_layer_shift,center+base_layer_shift]

third_layers=[]
for i in range(3):
	for j in range(2):
		third_layers.append([third_layer[j]+(i-1)*x_offset])
third_layers=np.array(third_layers).reshape((3,2,3))
...
for i in range(len(third_layers)):
	q_init=np.degrees(robot.inv(third_layers[i][0],R,q_seed)[0])

	q_end=np.degrees(robot.inv(third_layers[i][1],R,q_seed)[0])
